...
class Zalo():
    ...
    def get_list_users_id(self, token):
        ...
        url = "https://openapi.zalo.me/v2.0/oa/getfollowers?data=" + data + "&access_token=" + token
    ...
    def get_user_info(self, token):
        ...
        try:
            ...
            for i in list_users:
                ...
                url = "https://openapi.zalo.me/v2.0/oa/getprofile?data=" + data + "&access_token=" + token
                try:
                    _r = requests.get(url)
                    respone = json.loads(_r.text)
                    user_data = {"user_id": respone['data']['user_id'], "user_name": respone['data']['display_name'], "user_gender": respone['data']['user_gender']}
                    try:
                        if not self.check_user_exists(respone['data']['user_id']):
                            zoa_users.insert_one(user_data)
                            print("User", respone['data']['display_name'], "has been added to database.")
                        else:
                            print("User", respone['data']['display_name'],"exists!")
                    except ConnectionError:
                        print("Database Error!")
                except ConnectionError:
                    print("Connection Error!")
    ...
    def save_messages(self, token):
        ...
        for i in zoa_users.find({}, {"user_id": 1}):
            ...
            url = "https://openapi.zalo.me/v2.0/oa/conversation?data=" + json.dumps(data) + "&access_token=" + token
            try:
                _r = requests.get(url)
                respone = json.loads(_r.text)
                ...
                for j in conversation:
                    if self.check_message_exist(j['message_id']):
                        ...
                        print("Message", j['message_id'], 'has been added to database.')
            except ConnectionError:
        ...
        try:
            ...
            if replied_msg.count_documents({"message_id": lastest_message['message_id']}) == 0:
                if lastest_message['src'] == 1 and lastest_message['type']=="text":
                    if lastest_message['message'] == "#see_more":
                        self.send_image(token, lastest_message['from_id'])
                    else:
                        url = "http://127.0.0.1:5005/webhooks/rest/webhook"
                        data_dict = {"sender": lastest_message['from_display_name'], "message": lastest_message['message']}
                        data = json.dumps(data_dict)
                        headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
                        try:
                            _r_bot = requests.post(url, data, headers)
                            _respone_bot = json.loads(_r_bot.text)
                            self.send_text_message(token, lastest_message['from_id'], _respone_bot[0]['text'])
                            replied_msg.insert_one({"message_id": lastest_message['message_id']})
                        except ConnectionError:
